...
| setChannels | bool | Optional | False | Unsubscribe to channels, Either channel or channelGroup is required
| setChannelGroups | bool | Optional | false | Unsubscribe to channel groups, Either channel or channelGroup is required
|===
...
.1. Returning State
----
$result = $pubnub->hereNow()->channels("my_channel")->inclideUuids(true)->includeState(true)->sync();
----
...
----
$result = $pubnub->hereNow()->channels("my_channel")->inclideUuids(false)->includeState(false)->sync();
----
...
----
$result = $pubnub->hereNow()->inclideUuids(true)->includeState(false)->sync();
----
...
----
$result = $pubnub->hereNow()->inclideUuids(true)->includeState(true)->sync();
----
...
| setState | array | Optional | `state` to set
| getState | array | Optional | `state` to get
|===
...
|===
| Method | Type | Description
| getLevel() | str | Permissions level, one of `subkey`, `subkey+auth`, `channel`, 'channel-group',
...
|===
| Parameter | Type | Required | Description
| channels | str|array | Yes | `channels` to add to the channel group
| channelGroup | str | Yes | The `channel group` to add the channels to
|===
...
|===
| Parameter | Type | Required | Description
| channelGroup | str | Yes | The `channel group` to fetch channels
|===
...
|===
| Parameter | Type | Required | Description
| channels | str|array | Yes | `channels` remove from the channel group.
| channelGroup | str | Yes | The `channel group` to remove the channels from.
|===
...
|===
| Parameter | Type | Required | Description
| channelGroup | str | Yes | The `channel group` to remove
|===
...
$pubnub->arrayPushProvisions()->pushType(PNPushType)->deviceId(string)->sync();
----
...
| getTimetoken() | date | false | | Returns a `date` representation of current time token.
|===
